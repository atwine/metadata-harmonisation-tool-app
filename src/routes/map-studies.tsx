...
function MapStudiesPage() {
  ...
  // back to a normal, submittable gap even though `r.already_submitted`
  // (a snapshot from the lookup call) still says otherwise.
  const [afpoUnstuckGaps, setAfpoUnstuckGaps] = useState<Set<string>>(new Set());
  const [afpoRecheck, setAfpoRecheck] = useState<Record<string, AfpoGithubCheckState>>({});
  ...
  const proceedWithAfpoSubmit = (originalValue: string, submittedAs: string) => {
    ...
    // Deliberately no noopener/noreferrer: those make window.open() return
    // null in modern browsers (the whole point is to sever the scriptable
    // reference), which would silently leave this tab on about:blank forever
    // since popup.location.href below could never run. Safe to omit here —
    ...
    // A prior recheck may have unstuck this term (see handleAfpoRecheck) —
    // clear that now so `alreadySubmitted` reflects this fresh submission
    // instead of staying permanently false from the earlier unstick.
    setAfpoUnstuckGaps((prev) => {
    ...
    setAfpoRecheck((prev) => {
      const next = { ...prev };
      delete next[originalValue];
      return next;
    });
  };

  // Shared by handleAfpoSubmitGap and handleAfpoRecheck below — both need
  // "ask GitHub whether this term already has an issue" and a graceful
  // fallback if the check itself fails (rate-limited, offline, etc).
  const checkGithubFor = async (term: string, force = false): Promise<GithubCheckResponse> => {
    try {
      return await api.afpoCheckGithub(term, force);
    } catch {
      return { status: "unavailable", issues: [], from_cache: false };
    }
  };
  ...
  // The recovery path for a local "already submitted" flag that turns out to
  // be wrong (e.g. a submission whose tab never actually reached GitHub).
  // Only clears the flag when GitHub confirms no matching issue exists at
  // all — if one does (open or closed), something real was filed for this
  // term by us or another installation, so the block should stay in place.
  const handleAfpoRecheck = async (originalValue: string) => {
    if (!study || !selectedVar) return;
    const term = (afpoGapEdits[originalValue] || originalValue).trim() || originalValue;

    setAfpoRecheck((prev) => ({ ...prev, [originalValue]: { status: "checking" } }));
    // force=true: this is a user-initiated "is it REALLY still missing"
    // check — serving the same 24h cache the original submit-time check
    // used could hand back a stale "none" for a term that was actually
    // just filed, letting a genuine duplicate through.
    const result = await checkGithubFor(term, true);
    setAfpoRecheck((prev) => ({ ...prev, [originalValue]: { ...result, checkedTerm: term } }));
    ...
    if (result.status === "none") {
      ...
      setAfpoUnstuckGaps((prev) => new Set(prev).add(originalValue));
      setAfpoSubmittedGaps((prev) => {
        const next = new Set(prev);
        next.delete(originalValue);
        return next;
      });
