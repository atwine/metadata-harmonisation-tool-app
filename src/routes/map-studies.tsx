...
/** Client-side mirror of the backend's build_github_issue_url, so editing a
 * gap's "submit as" term updates the link live without a round trip. */
function buildAfpoGithubIssueUrl(unmatchedValue: string, studyName: string, variableName: string): string {
  const title = `New term request: ${unmatchedValue}`;
  const body = `## New AfPO Term Request
...
`;
  const params = new URLSearchParams({ title, body, labels: "new term" });
  return `https://github.com/h3abionet/afpo/issues/new?${params.toString()}`;
}
...
function MapStudiesPage() {
  ...
  const handleAfpoSubmitGap = (originalValue: string) => {
    ...
    const url = buildAfpoGithubIssueUrl(submittedAs, study, selectedVar);
